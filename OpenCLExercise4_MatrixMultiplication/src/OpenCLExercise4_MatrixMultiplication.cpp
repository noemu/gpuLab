...
int main(int argc, char** argv) {
	...
	cl::Buffer d_input_A (context, CL_MEM_READ_WRITE, sizeA);
	cl::Buffer d_input_B (context, CL_MEM_READ_WRITE, sizeB);
	cl::Buffer d_output_C (context, CL_MEM_READ_WRITE, sizeC);
	...
	queue.enqueueWriteBuffer(d_output_C,true,0,sizeC,h_outputCGpu.data());
	...
	*/
	...
	Core::TimeSpan cpuTime = cpuEnd - cpuStart; //TODO
	...
	queue.enqueueWriteBuffer(d_input_A,true,0,sizeA,h_inputA.data());
	queue.enqueueWriteBuffer(d_input_B,true,0,sizeB,h_inputB.data());
	...
	for (int impl = 1; impl <= 2; impl++) {
		...
		cl::Event execution;
		matrixMulKernel.setArg<cl::Buffer>(0, d_input_A);
		matrixMulKernel.setArg<cl::Buffer>(1, d_input_B);
		matrixMulKernel.setArg<cl::Buffer>(2, d_output_C);
		matrixMulKernel.setArg<std::size_t>(3, countAX_BY);
		queue.enqueueNDRangeKernel(matrixMulKernel, cl::NullRange, cl::NDRange(countCX, countCY), cl::NDRange(wgSize, wgSize), NULL, &execution);


		// Copy output data back to host
		cl::Event copy2;
		queue.enqueueReadBuffer(d_output_C, true, 0, sizeC, h_outputCGpu.data(), NULL, &copy2);

		// Print performance data
		Core::TimeSpan gpuTime =  OpenCL::getElapsedTime(execution); //TODO
		Core::TimeSpan copyTime =  OpenCL::getElapsedTime(copy2); //TODO
