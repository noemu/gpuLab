...
int main(int argc, char** argv) {
	...
	cl::Kernel kernel2(program, "blockAddKernel");
	...
	cl::Event copy2;
	queue.enqueueReadBuffer(d_output, true, 0, wgSize*wgSize*wgSize*sizeof(cl_int), h_outputGpu.data(), NULL, &copy2);
	queue.enqueueReadBuffer(d_sum, true, 0, wgSize*wgSize*sizeof(cl_int) , h_temp1.data(), NULL, &copy2);


	kernel1.setArg<cl::Buffer>(0, d_sum);
	kernel1.setArg<cl::Buffer>(1, d_sum);
	kernel1.setArg<cl::Buffer>(2, d_sum2);
	queue.enqueueNDRangeKernel(kernel1, 0, wgSize*wgSize, wgSize, NULL, &execution);

	queue.enqueueReadBuffer(d_sum, true, 0, wgSize*wgSize*sizeof(cl_int), h_temp1.data(), NULL, &copy2);
	queue.enqueueReadBuffer(d_sum2, true, 0, wgSize*sizeof(cl_int) , h_temp2.data(), NULL, &copy2);

	kernel1.setArg<cl::Buffer>(0, d_sum2);
	kernel1.setArg<cl::Buffer>(1, d_sum2);
	kernel1.setArg<cl::Buffer>(2, cl::Buffer());
	queue.enqueueNDRangeKernel(kernel1, 0, wgSize*wgSize, wgSize, NULL, &execution);

	// Call the kernels
	kernel2.setArg<cl::Buffer>(0, d_sum);
	kernel2.setArg<cl::Buffer>(1, d_sum2);
	kernel2.setArg<cl::Buffer>(2, d_sum);
	queue.enqueueNDRangeKernel(kernel2, 0, wgSize*wgSize, wgSize, NULL, &execution);

	// Call the kernels
	kernel2.setArg<cl::Buffer>(0, d_output);
	kernel2.setArg<cl::Buffer>(1, d_sum);
	kernel2.setArg<cl::Buffer>(2, d_output);
