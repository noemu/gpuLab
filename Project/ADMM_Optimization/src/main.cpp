...
int main(int argc, char** argv)
{
    ...
    //std::vector<viennacl::vector<T> > vcl_src;
    //viennacl::vector<T> vcl_dst;
    ...
    Eigen::SparseMatrix<float, Eigen::RowMajor, int> AT;
    Eigen::SparseMatrix<float, Eigen::RowMajor, int> A;
    Eigen::SparseMatrix<float, Eigen::RowMajor, int> AT2;
    Eigen::SparseMatrix<float, Eigen::RowMajor, int> A2;
    ...
    for (size_t i = 0;i < image_count;i++)
    {
        Src[i] = cv::imread("../ibm_4_300/LR"+ boost::lexical_cast<std::string> (i+1) + ".tif", CV_LOAD_IMAGE_ANYDEPTH);
	...
	resize(Src[0], dest, dest.size(), 0, 0, INTER_CUBIC);
    ...
    /***** ToDo: Implement ADMM optimization to solve given cost function, including calculate the gradient... ******/
