...
int main(int argc, char** argv) {
	...
	std::size_t countX = wgSizeX * 400; // Overall number of work items in X direction = Number of elements in X direction
	...
	cl::Image2D image(context,	CL_MEM_READ_ONLY, cl::ImageFormat(CL_R,CL_FLOAT),countX,countY);
	...
	*/
	...
	for (int impl = 1; impl <= 2; impl++) {
		...
		region[1] = countY;
		region[2] = 1;
		cl::Event copyToClient;
		queue.enqueueWriteImage(image,true, origin,region,countX *sizeof(float), 0,&(h_input[0]),NULL,&copyToClient);
		...
		sobelKernel.setArg<cl_uint>(1, countX);
		sobelKernel.setArg<cl_uint>(2, countY);
		sobelKernel.setArg<cl::Buffer>(3, d_output);
		...
		Core::TimeSpan overallGpuTime = OpenCL::getElapsedTime(execution) + OpenCL::getElapsedTime(copyToHost)+OpenCL::getElapsedTime(copyToClient);
		Core::TimeSpan overallCpuTime = cpuEnd - cpuStart;

		std::cout << "CPU Time: " << overallCpuTime.toString() <<" --- "<< (countX*countY/pow(10.0,6))/ overallCpuTime.getSeconds() <<" MPixel/s"<< std::endl ;
		std::cout << "GPU overall Time: " << overallGpuTime.toString() << " --- "<< (countX*countY/pow(10.0,6))/ overallGpuTime.getSeconds() <<" MPixel/s"<< std::endl ;
		std::cout << "GPU execution Time: " << OpenCL::getElapsedTime(execution).toString() << " --- "<< (countX*countY/pow(10.0,6))/ OpenCL::getElapsedTime(execution).getSeconds() <<" MPixel/s"<< std::endl ;
