...
int main(int argc, char** argv) {
	...
	glutCreateWindow("OpenCL exercise 5: Volume rendering");
	...
	cl::Program program = OpenCL::loadProgramSource(context, "src/OpenCLExercise5_VolumeRendering.cl");
	...
	d_invViewMatrix = cl::Buffer(context,CL_MEM_READ_ONLY,16*sizeof (float));
	...
	//TODO
	renderKernel = cl::Kernel(program, "kernel1");
	...
	region[1] = countY;
	region[2] = countZ;
	queue.enqueueWriteImage (d_input,true, origin,region,countX * sizeof(float), countX * countY * sizeof (float),(void*) h_input, NULL, &copyToDev);
...
void render () {
	...
	if (runGpu) {
		...
		cl::Event copy1;
		queue.enqueueWriteBuffer(d_invViewMatrix, true, 0, 16*sizeof(float), (void*)invViewMatrix, NULL, &copy1);

		// Call the kernel
		cl::Event execution;
		renderKernel.setArg<cl::Image3D>(0, d_input);
		renderKernel.setArg<cl::Buffer>(1,d_invViewMatrix),
		renderKernel.setArg<float>(2, tstep);
		renderKernel.setArg<cl::Buffer>(3, d_output);

		queue.enqueueNDRangeKernel(renderKernel, cl::NullRange, cl::NDRange(outX, outY), cl::NullRange, NULL, &execution);
		...
		//TODO
		cl::Event copy2;
		queue.enqueueReadBuffer(d_output,true,0,sizeOutput,h_outputGpu.data(),NULL, &copy2);
