...
void CPUImplementation::execute(float T1 = 0.1, float T2 = 0.7) {
    ...
    Core::writeImagePGM("output_Gauss_CPU.pgm", h_input, imageWidth, imageHeight);
    ...
    Core::writeImagePGM("output_Gradient_CPU.pgm", h_magnitude, imageWidth, imageHeight);
    //
    // Non Maximum Suppression
    //
    nonMaximumSuppression(h_magnitude, h_direction, non_max_sup.data());

    Core::writeImagePGM("output_NonMaxSup_CPU.pgm", non_max_sup, imageWidth, imageHeight);
    //
    // Hysterese Kernel
    //
    h_input = non_max_sup;
    ...
    Core::writeImagePGM("output_CannyEdge_CPU.pgm", h_outputCpu, imageWidth, imageHeight);
}
...
void CPUImplementation::printTimeMeasurement(Core::TimeSpan cpuExecutionTime) {
    ...
    outputString << std::left << std::setw(firstColumnWidth) << "CPU-Performance" << std::setw(generalColumnWidth)
                 << "execution" << std::endl;
    outputString << std::left << std::setw(firstColumnWidth) << "" << std::setw(generalColumnWidth)
                 << cpuExecutionTime.getSeconds() << std::endl;
...
float CPUImplementation::getValueGlobal(const std::vector<float>& a, int x, int y) {
    if (x < 0) x = 0;
...
void CPUImplementation::sobelHost(float* h_direction, float* h_outputCpu) {
    for (int i = 0; i < imageWidth; i++) {
        for (int j = 0; j < imageHeight; j++) {
            float Gx = getValueGlobal(h_input, i - 1, j - 1) + 2 * getValueGlobal(h_input, i - 1, j) +
                       getValueGlobal(h_input, i - 1, j + 1) - getValueGlobal(h_input, i + 1, j - 1) -
                       2 * getValueGlobal(h_input, i + 1, j) - getValueGlobal(h_input, i + 1, j + 1);
            float Gy = getValueGlobal(h_input, i - 1, j - 1) + 2 * getValueGlobal(h_input, i, j - 1) +
                       getValueGlobal(h_input, i + 1, j - 1) - getValueGlobal(h_input, i - 1, j + 1) -
                       2 * getValueGlobal(h_input, i, j + 1) - getValueGlobal(h_input, i + 1, j + 1);
...
void CPUImplementation::nonMaximumSuppressor(
    std::vector<float> l_Strength, float* h_output, float strength, int x, int y, int a_x, int a_y) {

    int leftIndex = (x + a_x) + (imageWidth) * (y + a_y);
    int rightIndex = (x - a_x) + (imageWidth) * (y - a_y);

    // Non Maximum Suppression
    float strengthA = getValueGlobal(l_Strength, x + a_x, y + a_y);
    float strengthB = getValueGlobal(l_Strength, x - a_x, y - a_y);
...
void CPUImplementation::nonMaximumSuppression(
    std::vector<float> h_input_Strength, std::vector<float> h_input_Direction, float* h_output) {
    for (int x = 0; x < imageWidth; x++) {
        for (int y = 0; y < imageHeight; y++) {
            ...
            if ((alpha > -1.0 * pi_8 && alpha < pi_8) || (alpha > 7.0 * pi_8 && alpha < PI) ||
                ...
                // l or r
                nonMaximumSuppressor(h_input_Strength, h_output, strength, x, y, 1, 0);
            }
            if ((alpha > pi_8 && alpha < 3.0 * pi_8) || (alpha >= -7.0 * pi_8 && alpha < -5.0 * pi_8)) {
                // tr(top right) or bl(bottom left
                nonMaximumSuppressor(h_input_Strength, h_output, strength, x, y, 1, 1);
            }
            if ((alpha > 3.0 * pi_8 && alpha < 5.0 * pi_8) || (alpha > -5.0 * pi_8 && alpha < -3.0 * pi_8)) {
                // t or b
                nonMaximumSuppressor(h_input_Strength, h_output, strength, x, y, 0, 1);
            }
...
void CPUImplementation::followEdge(int2 lastDirection, int2 pos, float* h_output, float T1, float T2) {
    bool finished = false;
    int2 directions[8] = {
        int2(0, 1), int2(1, 0), int2(0, -1), int2(-1, 0), int2(1, 1), int2(-1, -1), int2(-1, 1), int2(1, -1)};
    ...
    if (!finished) h_output[pos.x + imageWidth * pos.y] = 1;
}
